...
func (k Keeper) SetLogs(txHash common.Hash, logs []*ethtypes.Log) {
	...
	//fmt.Println("txHash.Bytes():",txHash.Bytes())
	//fmt.Println("bz:",bz)
	store.Set(txHash.Bytes(), bz)
}

// SetContractAddress
func (k Keeper) SetContractAddress(addr string) {
	store := prefix.NewStore(k.Ctx().KVStore(k.storeKey), types.KeyPrefixContractAddress)
	store.Set([]byte(types.KeyOfContractAddress),[]byte(addr))
	fmt.Println("SetContractAddress::::::",addr)
	fmt.Println("GetContractAddress::::::",k.GetContractAddress())
}

// GetContractAddress
func (k Keeper) GetContractAddress() string{
	store := prefix.NewStore(k.Ctx().KVStore(k.storeKey), types.KeyPrefixContractAddress)
	add := store.Get([]byte(types.KeyOfContractAddress))
	return string(add)
}

// UpdateContractAddress
func (k Keeper) UpdateContractAddress(addr string) {
	store := prefix.NewStore(k.Ctx().KVStore(k.storeKey), types.KeyPrefixContractAddress)
	store.Delete([]byte(types.KeyOfContractAddress))
	store.Set([]byte(types.KeyOfContractAddress),[]byte(addr))
}

// SetValidatorAddresses
func (k Keeper) SetValidatorAddresses(addr []string){
	store := prefix.NewStore(k.Ctx().KVStore(k.storeKey), types.KeyPrefixValidatorAddresses)
	json,e := json.Marshal(addr)
	if e != nil {
		return
	}
	store.Set([]byte(types.KeyOfValidatorAddresses),json)
}
// GetAddresses return addresses for staking.
func (k *Keeper) GetValidatorAddresses() []string{
	//store := prefix.NewStore(k.Ctx().KVStore(k.storeKey), types.KeyPrefixContractAddress)
	//data := store.Get([]byte(types.KeyOfValidatorAddresses))
	//var addresses []string
	//e := json.Unmarshal(data,&addresses)
	//if e != nil {
	//	return nil
	//}
	// mock
	valAddresses = []string{
		"eth1etzxe9wae6fka0jmg42tu86qflwnpq80wq2vf8",
		"eth1etzxe9wae6fka0jmg42tu86qflwnpq80wq2vf9",
	}
	return valAddresses
}
