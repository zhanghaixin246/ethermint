...
$ %s debug addr 0xA588C66983a81e800Db4dF74564F09f91c026351`, version.AppName, version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			addrString := args[0]
			cfg := sdk.GetConfig()
			fmt.Println(common.IsHexAddress(addrString))
			fmt.Println(strings.HasPrefix(addrString, cfg.GetBech32ValidatorAddrPrefix()))
			fmt.Println(strings.HasPrefix(addrString, cfg.GetBech32AccountAddrPrefix()))
			...
			case strings.HasPrefix(addrString, cfg.GetBech32AccountAddrPrefix()):
				...
				fmt.Println("addr:",string(addr))
				fmt.Printf("Bech32 Val: %s\n", sdk.ValAddress(addr))
				fmt.Println("sdk.ValAddress(addr):",sdk.ValAddress(addr))
